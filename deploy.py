...
class InferenceCore(nn.Module):
    def __init__(self, network: LIVOS) -> None:
        ...
        self.state_BNCC = torch.empty(0)
        self.key_sum_BCHW = torch.empty(0)
        self.sensory_BNCHW = torch.empty(0)
        self.obj_mem_sum_BNQC = torch.empty(0)
        self.last_masks_BNHW = torch.empty(0)
    ...
    def forward(self, image_BCHW, mask_HW, objects):
        ...
        if objects.numel() > 0:
            ...
            self.sensory_BNCHW = torch.zeros(B, N, C, H, W, device=image_BCHW.device)

            # Encode mask.
            mask_BNHW = mask_NHW.unsqueeze(0)
            value_BNCHW, self.sensory_BNCHW, obj_mem_BNQC = self.network.encode_mask(
                image_BCHW, pixfeat_BCHW, mask_BNHW, self.sensory_BNCHW, deep_update=True)
            ...
            self.sensory_BNCHW = self.sensory_BNCHW
...
def main():
    with torch.inference_mode():
        ...
        livos = torch.jit.script(InferenceCore(network).cuda().eval())
        livos = torch.jit.optimize_for_inference(livos)
        livos.save('weights/livos-wmose-480p-scripted.pth')
